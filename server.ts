...
async function startServer() {
  ...
  app.post("/api/telemetry", (req: Request, res: Response) => {
    ...
    // Store in historical record (only once per minute to avoid huge files & lag)
    ...
    saveData();
    try { fs.writeFileSync(HISTORY_FILE, '[]', 'utf8'); } catch(e) {}
...
app.delete("/api/recipes/:name", (req: Request, res: Response) => {
  ...
  // 10. Webcam stream
  app.get("/api/webcam/info", (req: Request, res: Response) => {
